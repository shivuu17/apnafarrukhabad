...
function Header({ scrolled }) {
  ...
  const headerTemp = typeof weather?.temperature === 'number' ? `${Math.round(weather.temperature)}°` : '...'
  const headerWeatherIcon = getWeatherDisplayIcon(weather?.description, weather?.conditionCode)
      ...
      <div className="bg-[#06391c] text-white text-sm">
        <div className="mx-auto flex max-w-6xl flex-col gap-1 px-3 py-2 sm:flex-row sm:items-center sm:justify-between sm:px-4 sm:py-1.5">
          ...
          <div className="sm:hidden">
            <div className="flex items-center justify-between gap-2 rounded-2xl border border-white/15 bg-white/10 px-3 py-2 text-[12px] font-semibold text-white/95 backdrop-blur-sm">
              <div className="flex min-w-0 items-center gap-2">
                <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-white/15 text-sm">
                  {headerWeatherIcon}
                </span>
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-extrabold">{headerTemp}</span>
                    <span className="truncate text-white/80">{weather?.description || t('weatherToday')}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="hidden items-center gap-4 text-[13px] sm:flex">
            <div className="flex items-center gap-2">
              <span>{headerWeatherIcon}</span>
              <span>{t('weatherToday')} {headerTemp}</span>
            </div>
