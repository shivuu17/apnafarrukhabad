          ...
          {villages.map((item, index) => (
            ...
            >
              ...
              <div className="absolute inset-x-3 bottom-3 sm:inset-x-4 sm:bottom-4 rounded-xl border border-white/30 bg-black/40 p-3 sm:p-4 backdrop-blur-md">
                <p className="text-base sm:text-lg font-extrabold text-white">{''}</p>
                <p className="text-xs sm:text-sm font-medium text-slate-200 mt-0.5">{''}</p>
                <div className="mt-2 flex items-center justify-between">
                  <span className="text-xs sm:text-sm font-bold text-white">{''}</span>
                  <span className="rounded-full bg-saffron-500 px-2.5 py-1 text-xs sm:text-[10px] font-extrabold text-white">{''}</span>
                </div>
              </div>
            </motion.button>
          ))}
